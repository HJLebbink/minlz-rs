//! Differential test: the AVX-512 (VBMI2) structural-decode kernel
//! (`asm/decompress_avx512.asm`) must produce byte-identical output to the scalar
//! `decompress_block` oracle, over inputs that exercise the LZ token paths
//! (short/long literals, 16- and 24-bit offsets, short/long matches, last-offset
//! reuse, overlapped copies). On non-AVX-512 hosts `iguana_decompress_simd`
//! transparently falls back to scalar, so this still passes (trivially).
...
fn check(input: &[u8]) {
    ...
    assert_eq!(
        scalar,
        input,
        "scalar round-trip failed ({} B)",
        input.len()
    );
    let simd = iguana_decompress_simd(&framed).expect("simd decode");
    assert_eq!(simd, input, "AVX-512 decode != input ({} B)", input.len());
    assert_eq!(simd, scalar, "AVX-512 decode != scalar ({} B)", input.len());
}

#[test]
fn avx512_decompress_matches_scalar() {
    ...
    check(&vec![0u8; 9000]);
    // Long literal runs (pseudo-random, few matches -> long literals).
    ...
    // path (token_count & 0x3f).
    for n in [64usize, 65, 127, 128, 200, 511, 512, 1000, 4096, 4097] {
        let data: Vec<u8> = (0..n)
